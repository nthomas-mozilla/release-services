...
initAddTreeFields =
    [ ( "name", Form.Input.textInput ) ]


initUpdateTreeFields : List ( String, Form.Field.Field )
initUpdateTreeFields =
    [ ( "status", Form.Input.textInput )
    , ( "reason", Form.Input.textInput )
    , ( "message_of_the_day", Form.Input.textInput )
    , ( "tags"
      , App.TreeStatus.Types.possibleTreeTags
            |> List.map (\( _, x, _ ) -> ( x, Form.Input.checkboxInput False ))
            |> Form.Field.group
      )
    , ( "remember", Form.Input.checkboxInput True )
    ]
